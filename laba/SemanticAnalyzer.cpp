...
void SymbolTable::insert(const std::string& name, const VariableInfo& info) {
    if (count < 100) {
        table[count].name = name;
        table[count].info = info;
        table[count].occupied = true;
        count++;
    }
}

VariableInfo* SymbolTable::find(const std::string& name) {
    for (int i = 0; i < count; i++) {
...
void SemanticAnalyzer::printErrors() {
    if (errorMessages.size() > 0) {
        outputFile << "\nSemantic errors (vsego: " << errorMessages.size() << "):\n";
...
void SemanticAnalyzer::checkTypeCompatibility(const std::string& expected,
    ...
    if (expected != actual) {
        error("Nesovmestimost' tipov: ozhidayetsya " + expected +
            ", poluchen " + actual + (context.empty() ? "" : " â " + context),
            line, position);
    }
}
...
bool SemanticAnalyzer::analyze(ParseTreeNode* root) {
    ...
    StringSet newUsedVars;
    StringSet newDeclaredVars;
    StringStack newTypeStack;
    ErrorList newErrorList;

    globalSymbolTable = newSymbolTable;
    usedVariables = newUsedVars;
    declaredVariables = newDeclaredVars;
    typeStack = newTypeStack;
    errorMessages = newErrorList;
...
void SemanticAnalyzer::traverseVarList(ParseTreeNode* node, const std::string& type) {
    ...
    for (int i = 0; i < varCount; i++) {
        ...
        if (globalSymbolTable.contains(vars[i])) {
            error("Povtornoye obyavleniye peremennoy '" + vars[i] + "'",
                lines[i], positions[i]);
        }
        else {
            VariableInfo varInfo(vars[i], type, lines[i], positions[i]);
            globalSymbolTable.insert(vars[i], varInfo);
...
void SemanticAnalyzer::traverseAssignment(ParseTreeNode* node) {
    ...
    for (size_t i = 0; i < node->children.size(); i++) {
        ...
        if (child->name == "id") {
            ...
            VariableInfo* info = globalSymbolTable.find(leftVarName);
            if (info) {
                leftType = info->type;
            }
            break;
...
void SemanticAnalyzer::traverseSimpleExpr(ParseTreeNode* node) {
    ...
    for (size_t i = 0; i < node->children.size(); i++) {
        ...
        if (child->name == "id") {
            ...
            VariableInfo* info = globalSymbolTable.find(varName);
            if (info) {
